...
export const AIAssistantPanel: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#FAF9F5] select-none">
      {/* Header */}
      <div className="p-3.5 bg-white border-b border-[#E8E6DF] flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-xl bg-neutral-950 flex items-center justify-center text-white">
            <Sparkles className="w-3.5 h-3.5 text-[#D4B996]" />
          </div>
          <div>
            <h3 className="text-xs font-bold text-neutral-900">AERA Spatial AI</h3>
            <p className="text-[10px] text-emerald-700 font-semibold">Active Spatial Co-Pilot</p>
          </div>
        </div>
      </div>
          ...
          return (
            <div key={m.id} className={`space-y-1.5 ${isUser ? 'ml-6' : 'mr-2'}`}>
              <div className="flex items-center gap-1.5 text-[10px] text-neutral-400 font-mono">
                {!isUser && <Bot className="w-3 h-3 text-[#B26A4A]" />}
              ...
              <div
                className={`p-3.5 rounded-2xl text-xs leading-relaxed ${
                  isUser
                    ? 'bg-neutral-950 text-white rounded-tr-xs shadow-xs'
                    : 'bg-white border border-[#E8E6DF] text-neutral-800 rounded-tl-xs shadow-2xs'
                }`}
              >
                <p>{m.content}</p>

                {m.suggestions && m.suggestions.length > 0 && (
                  <div className="mt-3 pt-2.5 border-t border-[#E8E6DF] flex flex-wrap gap-1.5">
                    {m.suggestions.map((s) => (
                      <button
                        key={s.id}
                        onClick={() => handleActionClick(s)}
                        className="px-2.5 py-1 bg-[#FAF4ED] hover:bg-[#F3E5D4] text-[#8C5232] rounded-lg text-[11px] font-bold transition-colors flex items-center gap-1"
                      >
      ...
      {/* Contextual Pills */}
      <div className="p-2 border-t border-[#E8E6DF] bg-white overflow-x-auto flex gap-1.5">
        {quickPills.map((pill, i) => (
          <button
            key={i}
            onClick={() => handleSend(pill)}
            className="px-2.5 py-1 bg-[#F5F4EF] hover:bg-[#EAE6DD] text-neutral-700 rounded-lg text-[10px] font-semibold whitespace-nowrap transition-colors"
          >
            {pill}
          </button>
        ))}
      ...
      {/* Chat Input */}
      <div className="p-3 bg-white border-t border-[#E8E6DF]">
        <form
          onSubmit={(e) => {
            e.preventDefault();
            handleSend();
        ...
        >
          <input
            ...
            className="flex-1 px-3 py-2 rounded-xl border border-[#E8E6DF] text-xs bg-[#FBFBF9] focus:outline-none focus:border-neutral-900"
          />
          <button
            type="submit"
            disabled={!inputText.trim()}
            className="p-2 bg-neutral-950 hover:bg-neutral-800 text-white rounded-xl disabled:opacity-40 transition-colors shadow-2xs"
          >
